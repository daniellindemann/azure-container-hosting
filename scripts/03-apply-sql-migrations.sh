...
# script variables
sqlserver_starts_with='sql-azh-shared'
sqldb_starts_with='sqldb-beer-rating'
...
dotnet ef database update \
    ...
    --startup-project "${script_dir}/../src/Demo.BeerRating.Backend" \
    --context BeerDbContext -- --environment Production
echo "🍰 Migrations applied"

echo "🌱 Seed data"
ASPNETCORE_ENVIRONMENT=Production dotnet run \
    --project "${script_dir}/../src/Demo.BeerRating.Backend" \
    --environment Database__UseInMemoryDatabase=false \
    --environment Database__UseAutoMigration=false \
    --environment Database__UseDataSeeding=true \
    --environment ConnectionStrings__Beer="${sql_connection_string}" \
    --seed
echo "🪴 Data planted"
